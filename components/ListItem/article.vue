<template>
    <div>
        <template v-for="item in list">
            ...
            <div v-if="item.thumb.length === 4" :key="item.id" class="lqsblog-list">
                <div class="lqsblog-list-content py-lg-1">
                    <div class="lqsblog-list-body">
                        <div class="text-lg h-2x">
                            <nuxt-link class="lqsblog-list-title" :to="`/article/detail/${item.id}`">
                            {{ item.title }}
                            </nuxt-link>
                        </div>
                    </div>
                    <div class="lqsblog-list-footer d-flex">
                        ...
                        <span class="d-inline-block">
                            <nuxt-link class="text-muted" :to="`/article/${item.category.alias}`">
                                {{ item.category.name }}
                            </nuxt-link>
                        </span>
                    ...
                    <div class="lqsblog-list-images row row-sm mt-3">
                        ...
                        <div class="lqsblog-media">
                            <nuxt-link class="lqsblog-media-content" :to="`/article/detail/${item.id}`" :style="{'background-image': 'url(' + getHref(thumb) + ')'}"></nuxt-link>
                        </div>
                        </div>
                    </div>
                </div>
            ...
            <div v-else-if="item.thumb.length < 1" :key="item.id" class="lqsblog-list">
            ...
            <div v-else :key="item.id" class="lqsblog-list">
                <div class="lqsblog-list-content py-lg-2">
                ...
                <div class="lqsblog-media shadow">
                    <nuxt-link class="lqsblog-media-content" :to="`/article/detail/${item.id}`" :style="{'background-image': 'url(' + getHref(item.thumb[0]) + ')'}"></nuxt-link>
                </div>
                </div>
